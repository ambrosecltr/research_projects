...
/workspace/genome/
  ...
  runs/
    decoder-v3/
    development-code-v3/
    decoder-evaluation-v3.json
    compiler-v3/
    hidden-prediction-v3/
    hidden-runtime-v3/
    hidden-evaluation-v3.json
    hidden-lm-eval-v3.json
```
...
genome polypythia train-decoder \
  ...
  --output "$ROUND1_RUNS/decoder-v3" \
  --device cuda

genome polypythia fit-development-code \
  --config configs/polypythia_14m_round1.yaml \
  --corpus "$ROUND1_SEALED" \
  --shared-decoder "$ROUND1_RUNS/decoder-v3" \
  --output "$ROUND1_RUNS/development-code-v3" \
  --device cuda

genome polypythia evaluate-decoder \
  --corpus "$ROUND1_SEALED" \
  --shared-decoder "$ROUND1_RUNS/decoder-v3" \
  --development-code "$ROUND1_RUNS/development-code-v3" \
  ...
  --output "$ROUND1_RUNS/decoder-evaluation-v3.json" \
  --device cuda
```
...
genome polypythia train-compiler \
  ...
  --corpus "$ROUND1_SEALED" \
  --shared-decoder "$ROUND1_RUNS/decoder-v3" \
  --output "$ROUND1_RUNS/compiler-v3" \
  --device cuda

genome polypythia predict-hidden \
  --corpus "$ROUND1_SEALED" \
  --shared-decoder "$ROUND1_RUNS/decoder-v3" \
  --compiler "$ROUND1_RUNS/compiler-v3" \
  --output "$ROUND1_RUNS/hidden-prediction-v3" \
  --device cuda

genome polypythia execute-hidden \
  --corpus "$ROUND1_SEALED" \
  --shared-decoder "$ROUND1_RUNS/decoder-v3" \
  --prediction "$ROUND1_RUNS/hidden-prediction-v3" \
  --model-config "$ROUND1_DOWNLOAD/tokenizer/config.json" \
  --output "$ROUND1_RUNS/hidden-runtime-v3" \
  --device cuda
```
...
genome polypythia download \
  ...
  --output-root "$ROUND1_DOWNLOAD" \
  --reveal-hidden \
  --prediction-seal "$ROUND1_RUNS/hidden-prediction-v3/prediction_seal.json" \
  --runtime-execution "$ROUND1_RUNS/hidden-runtime-v3/manifest.json"
```
...
genome polypythia evaluate-hidden \
  ...
  --runtime-execution "$ROUND1_RUNS/hidden-runtime-v3" \
  --model-config "$ROUND1_DOWNLOAD/tokenizer/config.json" \
  --tokenizer "$ROUND1_DOWNLOAD/tokenizer" \
  --evaluation-texts "$GENOME_ROOT/data/evaluation-texts/texts.jsonl" \
  --output "$ROUND1_RUNS/hidden-evaluation-v3.json" \
  --device cuda
```
...
genome polypythia evaluate-lm-harness \
  ...
  --model-config "$ROUND1_DOWNLOAD/tokenizer/config.json" \
  --tokenizer "$ROUND1_DOWNLOAD/tokenizer" \
  --output "$ROUND1_RUNS/hidden-lm-eval-v3.json" \
  --device cuda
```
