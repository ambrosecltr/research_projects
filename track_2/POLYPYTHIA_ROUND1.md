...
The checked-in V4 configuration uses a learned structured prediction plus a canonical fp16
residual for every block. The residual is defined by the frozen decoder, not by arbitrary latent
optimization.
...
genome polypythia evaluate-decoder \
  ...
  --output "$ROUND1_RUNS/decoder-evaluation-v4.json" \
  --device cuda
```
...
genome polypythia execute-hidden \
  ...
  --model-config "$ROUND1_DOWNLOAD/tokenizer/config.json" \
  --output "$ROUND1_RUNS/hidden-runtime-v4" \
  --device cuda
```
...
genome polypythia download \
  ...
  --output-root "$ROUND1_DOWNLOAD" \
  --reveal-hidden \
  --prediction-seal "$ROUND1_RUNS/hidden-prediction-v4/prediction_seal.json" \
  --runtime-execution "$ROUND1_RUNS/hidden-runtime-v4/manifest.json"
```
...
genome polypythia evaluate-hidden \
  ...
  --tokenizer "$ROUND1_DOWNLOAD/tokenizer" \
  --evaluation-texts "$GENOME_ROOT/data/evaluation-texts/texts.jsonl" \
  --output "$ROUND1_RUNS/hidden-evaluation-v4.json" \
  --device cuda
```
...
genome polypythia evaluate-lm-harness \
  ...
  --model-config "$ROUND1_DOWNLOAD/tokenizer/config.json" \
  --tokenizer "$ROUND1_DOWNLOAD/tokenizer" \
  --output "$ROUND1_RUNS/hidden-lm-eval-v4.json" \
  --device cuda
```
